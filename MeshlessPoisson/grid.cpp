...
void Grid::setBCFlag(int bNum, std::string type, vector<double> boundValues) {
	...
	std::get<1>(bound) = type.compare("dirichlet") ? 1 : 2; 
...
void Grid::build_laplacian(int k) {
...
void Grid::sor(int numIts, Eigen::VectorXd source) {
	for (int it = 0; it < numIts; it++) {
		// set/enforce the boundary conditions
		boundaryOp();
		for (int i = 0; i < laplaceMatSize_; i++) {
			if (bcFlags_[i] != 0) {
				continue;
			}
			double x_i = (1-omega_)*(*values_)(i) + omega_/laplaceMat_->coeff(i,i)*source(i);
			for (int j = 0; j < laplaceMatSize_; j++) {
				x_i -= j == i ? 0 : laplaceMat_->coeff(i, j)*(*values_)(j);
			}
		}
	}
}
...
std::pair<Eigen::MatrixXd, vector<int>> Grid::buildCoeffMatrix(int pointID) {
	...
	//build A-matrix
	Eigen::MatrixXd coeff_mat = Eigen::MatrixXd(stencilSize_ + polyTerms, stencilSize_ + polyTerms);
	for (int i = 0; i < stencilSize_; i++) {
...
std::pair<Eigen::VectorXd, vector<int>> Grid::laplaceWeights (int pointID) {
	...
	int polyTerms = (polyDeg_ + 1)*(polyDeg_ + 2) / 2;
	Eigen::VectorXd rhs = Eigen::VectorXd(stencilSize_ + polyTerms);
	//Build RHS
